...
class NitradoApi:
    ...
    async def start_server(self,game="arksa"):
        ...
        payload = {'game': game}
        response = requests.post(f'https://api.nitrado.net/services/{self.SERVER_ID}/gameservers/games/start',
    ...
    async def uninstall_game(self,game="arksa"):
        ...
        payload = {'game': game}
        response = requests.delete(f'https://api.nitrado.net/services/{self.SERVER_ID}/gameservers/games/uninstall',
                                 auth=BearerAuth(self.NITRADO_TOKEN), params=payload)
        print("Status Code: " + str(response.status_code))
        print("Response: " + str(response.content))
        response_json = response.json()
    ...
    async def restart_server(self, message="Restarting...", restart_message="Restarting..."):
        ...
        response = requests.post(f'https://api.nitrado.net/services/{self.SERVER_ID}/gameservers/restart',
                                 auth=BearerAuth(self.NITRADO_TOKEN), params=payload)
        print("Status Code: " + str(response.status_code))
        print("Response: " + str(response.content))
        response_json = response.json()
